...
- 修改文件：
  - `AGENTS.md`（多设备登记表追加个人机）
  - `AGENTS.md_修改记录.md`（追加本条记录）
  - git 仓库级配置：设置本机作者名与统一 GitHub noreply 邮箱
- 修改内容：
  - 新设备主机名 `DESKTOP-FP4I9GS` 登记为「个人机」：用户 `chengwenjie`，平台 Windows，仓库路径 `E:\MyKnowledgeBase`，git 作者名 `大程·个人机`。
  - 仓库级 git 配置设置为 `大程·个人机 <cheng-wj@users.noreply.github.com>`，不影响全局 git 配置。
  - 将「知识库定位」中写死的 `F:\我的知识库` vault 路径改为设备无关表述，本机路径统一以多设备登记表为准。
- 修改思路/原因：
  - 本机不在家规设备表中，按「新设备首次接入流程」先自查主机名、用户名、平台和仓库路径，再向主人确认称呼。
  - 主人确认称呼为「个人机」后，补登设备表并设置独立 git 作者名，便于后续 README 与 log 用 `[@个人机]` 标注来源设备。
- 验证情况：
  - 已确认 hostname 为 `DESKTOP-FP4I9GS`，用户为 `chengwenjie`，系统为 Windows 10.0.19045，仓库路径为 `E:\MyKnowledgeBase`；
  - 设置后核对仓库级 `user.name` / `user.email`；首次提交后用 `git log -1 --pretty='%an <%ae>'` 确认最新提交作者。

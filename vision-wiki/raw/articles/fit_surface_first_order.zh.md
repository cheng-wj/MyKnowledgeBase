# fit_surface_first_order（算子）

**Name（名称）**

`fit_surface_first_order` — 计算灰度矩并用一阶曲面（平面）进行逼近。
...
算子 `fit_surface_first_order` 计算输入区域的**灰度矩**，并计算用于逼近这些灰度值的一阶曲面参数。计算方法为**最小化灰度值与曲面之间的距离**。

一阶曲面由下式给出：

$$
f(r, c) = \text{Alpha} \cdot (r - R) + \text{Beta} \cdot (c - C) + \text{Gamma}
$$

其中 `r`、`c` 是图像坐标，`R`、`C` 是输入区域与完整图像域相交部分的**中心坐标**。通过上述最小化过程即可求得 `Alpha` 到 `Gamma` 三个参数。

**拟合算法**可通过 `Algorithm` 参数选择：

- **`'regression'`**：标准的"最小二乘"直线拟合。
...
注意：算子 `fit_surface_first_order` **仅考虑给定的 `Regions`**，而**忽略**输入图像 `Image` 上先前设置的 domain（定义域）。
...
| `Alpha`（output_control） | real(-array) → (double) | 逼近曲面的参数 Alpha。 |
| `Beta`（output_control） | real(-array) → (double) | 逼近曲面的参数 Beta。 |
| `Gamma`（output_control） | real(-array) → (double) | 逼近曲面的参数 Gamma。 |

---

**Result（结果）**

如果输入的是带定义灰度值（`'byte'`）的图像，且参数正确，算子 `fit_surface_first_order` 返回值 `2`（`H_MSG_TRUE`）。如有必要会抛出异常。

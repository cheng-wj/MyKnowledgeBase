...
## [2026-09-08] ingest | 基恩士官方手册 + Matrox采集卡文档 + 埃科软件手册 + 书单 [@公司机]
- ① 基恩士 LJ-X8000A 官方手册（UM用户手册/RM参考手册/DLL使用指南/拍摄设定思路 + SDK示例）→ 原始资料/规格书/基恩士LJ-X8000A手册/ 与 代码/，传感器页加"官方手册"节。
- ② Matrox Grablink/MultiCam 采集卡文档（中文参数设置/硬件接线 + MultiCam/Grablink 英文手册9份）→ 原始资料/规格书/Matrox采集卡手册/，新建 [采集卡页](硬件/采集卡/线扫采集卡-Matrox-Grablink.md)（相机端/采集卡端参数、IIN帧信号/DIN行信号接线）。
- ③ 埃科相机配套软件手册 5 份（IKapExpert/CViewer/IPConfigurator/COMManager/固件升级）→ 原始资料/规格书/埃科相机软件手册/，埃科相机页加"配套软件手册"节。
- ④ 学习书籍只登记不入仓库：新建 [书单与学习资料](../书单与学习资料.md)（机器视觉算法与应用、C++ Primer、嵌入式Linux、Xilinx ZYNQ 等，记本地 F:\学习文档 路径）。
- 更新：index.md（硬件新增采集卡分组、分组标题去英文）、README、本日志。
